...
import { sharedWishlists, contacts } from "../../../dummyData";
...
export default function ViewWishlist() {
  ...
  let activeWishlist = sharedWishlists[wishlistID];

  let wishlistAuthor = contacts.find((c) => c.email === activeWishlist.owner);
      ...
      <ScrollView>
        <SafeAreaView style={{ gap: 16 }}>
          <View style={styles.infoRow}>
            <Text>Author:</Text>
            <Chip>
              {wishlistAuthor.firstName} {wishlistAuthor.lastName}
            </Chip>
          </View>
          <Divider />
          <View style={styles.infoRow}>
